...
@forelse ($grouped as $projectId => $items)
    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden mb-4">
        <div class="px-4 py-3 bg-slate-50 border-b border-slate-200 flex items-center gap-3 flex-wrap">
            <span class="font-bold text-slate-700">{{ __('app.project') }}: {{ $proj->name ?? '—' }}</span>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm {{ $rtl ? 'text-right' : 'text-left' }}">
                <thead class="bg-slate-50 text-slate-500 text-xs">
                    <tr>
                        <th class="px-4 py-2.5 w-28">{{ __('app.req_code') }}</th>
                        <th class="px-4 py-2.5">{{ __('app.req_item') }}</th>
                        <th class="px-4 py-2.5">{{ __('app.req_department') }}</th>
                        <th class="px-4 py-2.5">{{ __('app.req_responsible') }}</th>
                        <th class="px-4 py-2.5 text-center">{{ __('app.req_due') }}</th>
                        <th class="px-4 py-2.5">{{ __('app.status') }}</th>
                        <th class="px-4 py-2.5"></th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    @foreach ($items as $r)
                        <tr class="hover:bg-slate-50 {{ $r->isOverdue() ? 'bg-rose-50/30' : '' }}">
                            <td class="px-4 py-2.5 text-slate-400 text-xs whitespace-nowrap">{{ $r->code ?? '—' }}</td>
                            <td class="px-4 py-2.5">
                                <div class="font-medium text-slate-800">{{ $r->title }}</div>
                                @if ($r->note)<div class="text-xs text-slate-400">{{ $r->note }}</div>@endif
                            </td>
                            <td class="px-4 py-2.5 text-slate-500">{{ $r->departmentLabel() }}</td>
                            <td class="px-4 py-2.5">
                                <form method="POST" action="{{ route('requirements.update', $r) }}">
                                    @csrf @method('PATCH')<input type="hidden" name="title" value="{{ $r->title }}">
                                    <select name="assigned_to" onchange="this.form.submit()" class="text-xs rounded-lg border border-slate-200 px-2 py-1.5 bg-white">
                            ...
                            <td class="px-4 py-2.5 text-center whitespace-nowrap {{ $r->isOverdue() ? 'text-rose-600 font-semibold' : 'text-slate-500' }}">{{ $date($r->due_date) }}</td>
                            <td class="px-4 py-2.5">
                                <form method="POST" action="{{ route('requirements.update', $r) }}">
                                    @csrf @method('PATCH')<input type="hidden" name="title" value="{{ $r->title }}">
                                    <select name="status" onchange="this.form.submit()" class="text-xs rounded-lg border border-slate-300 px-2 py-1.5 bg-{{ $r->statusColor() }}-50 text-{{ $r->statusColor() }}-700">
                            ...
                            <td class="px-4 py-2.5">
                                <form method="POST" action="{{ route('requirements.destroy', $r) }}" onsubmit="return confirm('{{ __('app.confirm_delete') }}')">@csrf @method('DELETE')
                                    <button class="p-1.5 text-slate-300 hover:text-rose-600"><svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" d="M6 7h12M9 7V5h6v2m-7 0l1 12h6l1-12"/></svg></button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
